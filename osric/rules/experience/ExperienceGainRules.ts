/**
 * ExperienceGainRules - OSRIC Experience Point Calculation
 *
 * Migrated from rules/experience/monsterXP.ts and experienceManager.ts
 * PRESERVATION: All OSRIC experience calculation methods and values preserved exactly
 */

import type { Command } from '../../core/Command';
import type { GameContext } from '../../core/GameContext';
import { calculateMonsterXP } from '../../core/MonsterXP';
...
export class ExperienceGainRule extends BaseRule {
  ...
  readonly priority = 500; // Normal priority for experience calculations
  ...
  async execute(context: GameContext, _command: Command): Promise<RuleResult> {
    // For now, we get parameters from temporary data since Command interface doesn't expose them
    ...
    try {
      ...
      // Calculate base experience based on source
  ...
  /**
   * Calculate combat experience points from defeated monsters
   */
  private calculateCombatXP(
    ...
    for (const monster of monsters) {
      // Use character level for XP calculation (simplified to 1 for now)
      const monsterXP = calculateMonsterXP(monster);
      totalXP += monsterXP;
    }

    // Apply party sharing if enabled
  ...
  /**
   * Apply character-specific experience modifiers
   */
  private applyCharacterModifiers(
    ...
    // Prime requisite bonus (OSRIC rules)
    ...
    // Multi-class penalty (OSRIC rules)
    ...
    // Racial level limits (reduce XP gain near limits)
  ...
  /**
   * Get prime requisite experience bonus based on ability scores
   */
  private getPrimeRequisiteBonus(character: Character): number {
    // Get the prime requisite for the character's class
    const primeRequisiteScore = this.getPrimeRequisiteScore(character);

    // OSRIC prime requisite bonuses
    if (primeRequisiteScore >= 16) return 1.1; // +10%
    if (primeRequisiteScore >= 15) return 1.05; // +5%
    if (primeRequisiteScore >= 13) return 1.0; // No bonus
    if (primeRequisiteScore >= 9) return 1.0; // No penalty
    if (primeRequisiteScore >= 6) return 0.95; // -5%
    return 0.9; // -10%
  }

  /**
   * Get the prime requisite ability score for a character's class
   */
  private getPrimeRequisiteScore(character: Character): number {
    const characterClass = character.class.toLowerCase();

    // Handle multi-class characters (use highest prime requisite)
  ...
  /**
   * Get prime requisite score for a specific class
   */
  private getClassPrimeRequisite(characterClass: string, character: Character): number {
    ...
    switch (characterClass.toLowerCase()) {
      ...
      default:
        return 13; // Default to no bonus/penalty
    }
  }

  /**
   * Calculate multi-class experience penalty
   */
  private getMultiClassPenalty(character: Character): number {
    const classes = character.class.split('/');

    // OSRIC: Multi-class characters divide XP equally among classes
    // with additional penalties for some combinations
    let penalty = 1.0 / classes.length;

    // Additional penalties for certain combinations (simplified)
    if (classes.length > 2) {
      penalty *= 0.9; // Additional 10% penalty for triple-class
    }
  ...
  /**
   * Calculate racial level limit penalty (near level cap)
   */
  private getRacialLevelLimitPenalty(_character: Character): number {
    // This would need to be implemented based on current level
    // and racial level limits for the character's class
    // For now, return no penalty
    return 1.0;
  }
}
